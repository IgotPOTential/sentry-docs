...
Start by intalling `@next/sentry-source-maps` and `@zeit/next-source-maps`:

```bash {tabTitle:npm}
npm install --save @next/sentry-source-maps @zeit/next-source-maps
```

```bash {tabTitle:Yarn}
yarn add @next/sentry-source-maps @zeit/next-source-maps
```
